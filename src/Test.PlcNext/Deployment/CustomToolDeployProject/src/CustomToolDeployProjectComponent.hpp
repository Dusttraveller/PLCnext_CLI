...
//#acfcomponent
class CustomToolDeployProjectComponent : public ComponentBase, public MetaComponentBase, private Loggable<CustomToolDeployProjectComponent>
{
public: // construction/destruction
#if ARP_ABI_VERSION_MAJOR < 2
    CustomToolDeployProjectComponent(IApplication& application, const String& name);
    virtual ~CustomToolDeployProjectComponent() = default;
#else
    CustomToolDeployProjectComponent(ILibrary& library, const String& name);
#endif
...
private: // methods
#if ARP_ABI_VERSION_MAJOR < 2
    CustomToolDeployProjectComponent(const CustomToolDeployProjectComponent& arg) = delete;
    CustomToolDeployProjectComponent& operator= (const CustomToolDeployProjectComponent& arg) = delete;
...
#if ARP_ABI_VERSION_MAJOR < 2
inline IComponent::Ptr CustomToolDeployProjectComponent::Create(Arp::System::Acf::IApplication& application, const String& name)
{
    return IComponent::Ptr(new CustomToolDeployProjectComponent(application, name));
}
#endif
